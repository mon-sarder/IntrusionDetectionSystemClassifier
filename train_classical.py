"""
generate_data.py — NSL-KDD dataset loader / synthetic generator.
Drop KDDTrain+.txt (and optionally KDDTest+.txt) into data/ to use real data.
"""
import numpy as np
import pandas as pd
import os
...
SERVICES  = ['http','ftp','smtp','ssh','dns','pop3','nntp','telnet',
              'ftp_data','finger','auth','other','private']
FLAGS     = ['SF','S0','REJ','RSTO','RSTR','SH','S1','S2','S3','OTH']

_PW = [0.60, 0.30, 0.10]
_FW = [0.810,0.050,0.040,0.030,0.020,0.015,0.013,0.010,0.007,0.005]
_FW_DOS = [0.100,0.700,0.099,0.040,0.020,0.015,0.010,0.008,0.005,0.003]
...
def generate_synthetic(n_total=20000, seed=42):
    ...
    normal = _make(n_normal,'normal','Normal',(0,200),(100,60000),(100,60000),(0.00,0.05),(0.00,0.05),(0.70,1.00),(1,100))
    dos    = _make(n_dos,'dos','DoS',(0,5),(0,1500),(0,200),(0.70,1.00),(0.00,0.10),(0.90,1.00),(200,511),fw=_FW_DOS)
    dos['count'] = rng.randint(400, 511, n_dos)

    probe  = _make(n_probe,'probe','Probe',(0,30),(0,800),(0,800),(0.10,0.50),(0.30,0.80),(0.10,0.40),(1,60))
    probe['diff_srv_rate']             = rng.uniform(0.5, 1.0, n_probe).round(2)
    probe['dst_host_diff_srv_rate']    = rng.uniform(0.4, 1.0, n_probe).round(2)

    r2l    = _make(n_r2l,'r2l','R2L',(50,6000),(100,8000),(1000,150000),(0.00,0.10),(0.00,0.10),(0.40,0.90),(1,40))
    r2l['num_failed_logins'] = rng.randint(1, 6, n_r2l)
    r2l['logged_in']         = rng.choice([0,1], n_r2l, p=[0.3,0.7])

    u2r    = _make(n_u2r,'u2r','U2R',(100,12000),(1000,25000),(1000,25000),(0.00,0.05),(0.00,0.05),(0.30,0.80),(1,15))
...
def load_dataset(data_dir='data'):
    ...
    if os.path.exists(train_path):
        print(f"[data] Real NSL-KDD found → {train_path}")
        train, test = load_real_nslkdd(train_path, test_path if os.path.exists(test_path) else None)
        return train, test, 'real'
    else:
        print("[data] Using synthetic dataset (drop KDDTrain+.txt in data/ for real data)")
        df = generate_synthetic(n_total=20000)
        os.makedirs(data_dir, exist_ok=True)
        df.to_csv(os.path.join(data_dir, 'nsl_kdd_synthetic.csv'), index=False)
        return df, None, 'synthetic'


if __name__ == '__main__':
    train, _, src = load_dataset()
    print(f"Source: {src}  |  shape: {train.shape}")
